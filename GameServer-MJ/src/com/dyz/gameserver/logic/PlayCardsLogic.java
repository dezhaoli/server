...
public class PlayCardsLogic {
    ...
    public void setPlayerList(List<Avatar> playerList) {
        ...
        // 设置风位
        for (int i = 0; i < 4; i++) {
            Avatar avatar = this.playerList.get((pickAvatarIndex + i) % 4);
            avatar.avatarVO.setFengWei(i);
        }
    }
...
//		}
        ...
        if (roomVO.getRoomType() == 3) {
            ...
            if (chiAvatar.size() > 0 && (huAvatar.size() == 0 || (huAvatar.contains(avatar) && huAvatar.size() == 1))
                ...
                if (chiAvatar.contains(avatar)) {
                    ...
                    avatar.setCardListStatus(onePointIndex, 4);
                    avatar.setCardListStatus(twoPointIndex, 4);
                    ...
                    for (int i = 0; i < playerList.size(); i++) {
                        ...
                        playerList.get(i).getSession().sendMsg(new ChiResponse(1, cardIndex, onePointIndex,
                                twoPointIndex, playerList.indexOf(avatar)));
                    }
    ...
    public boolean pengCard(Avatar avatar, int cardIndex) {
        ...
        if ((penAvatar.size() >= 1 && huAvatar.size() == 0)
            ...
            if (penAvatar.contains(avatar)) {
                ...
                avatar.avatarVO.getPengArray()[cardIndex] = 1;
    ...
    public boolean gangCard(Avatar avatar, int cardPoint, int gangType) {
        ...
        // if(gangAvatar.size() > 0 && huAvatar.size() == 0 || (huAvatar.size() == 1 &&
        // huAvatar.contains(avatar) )) {//2016-8-1
                    ...
                    if (gangType == 0) {
                        ...
                        for (int i = 0; i < playerList.size(); i++) {
                            if (avatar.getUuId() != playerList.get(i).getUuId()) {
                                ...
                                // 杠牌返回给其他人只返回杠的类型和杠牌的玩家位置
                                playerList.get(i).getSession()
                                        .sendMsg(new GangResponse(1, cardPoint, avatarIndex, type));
...
//                    if (avatar.canHu) {
                    ...
                    if (!validMa.isEmpty()) {
                        ...
                        validMa = HuPaiType.getInstance().getHuType(avatar, avatar, roomVO.getRoomType(), cardIndex,
                                playerList, mas, huCount, "", roomVO.getHong(), fengQuan);
                    }
        ...
        if (huAvatar.size() == 0 && numb == 1) {
            ...
            if (mainHasHu) {
                ...
                int mainIndex = playerList.indexOf(bankerAvatar);
    ...
    public void settlementData(String type) {
        ...
        for (Avatar avatar : playerList) {
            ...
            score.append(
                    avatar.getUuId() + ":" + roomVO.getEndStatistics().get(avatar.getUuId() + "").get("scores") + ",");
            // 修改存储的分数(断线重连需要)
		...
		/*
		 * else{ 10-11注释 在游戏开始之后就已经重置准备属性为false //清除当前房间牌的数据信息 for (Avatar avatar :
		 * playerList) { avatar.avatarVO.setIsReady(false); } }
		 */
    }
    ...
    private boolean checkHu(Avatar avatar, Integer cardIndex) {
		 ...
		 * //有癞子时，直接进行癞子的胡牌判断，不需要进行单独的判断 int isSeven = checkSevenDouble(paiList);
		 * if(isSeven == 0){ System.out.println("没有七小对"); if(isHuPai(paiList)){
		 * System.out.print("胡牌"); //cleanPlayListCardData(); }else{
		 * System.out.println("checkHu 没有胡牌"); } }else{
		 * 
		 * if(isSeven == 1){ System.out.println("七对"); }else{ System.out.println("龙七对");
		 * } //cleanPlayListCardData(); return true; } } if(roomVO.getRoomType() == 1 &&
		 * roomVO.getHong()){ //转转麻将，可以选择红中 //红中当癞子 return Naizi.testHuiPai(paiList); }
		 * else{ return isHuPai(paiList); }
		 */
    }
    ...
    public boolean checkHuHShui(Avatar avatar, Integer cardIndex) {
		 ...
		 * if(!flag){
		 *//**
         * 1111 、11 、11 、11、 11 、11 （这种也算胡） 1111、1111、11、11、11（这种也算对胡） 1111 1111 1111
         * 11(也算对胡）
         *//*
				 * //特殊算法 if(!flag){ int twoCard = 0; int fourCard = 0; //1111 、11 、11 、11、 11
				 * 、11 for (int i = 0; i < paiList[0].length; i++) { if(paiList[0][i] ==2){
				 * twoCard++; } else if(paiList[0][i] ==4){ fourCard ++; } } if((twoCard == 5 &&
				 * fourCard == 1) || (twoCard == 3 && fourCard == 2) || (twoCard == 1 &&
				 * fourCard == 3)){ avatar.avatarVO.setHuType(1);//划水麻将小胡
    ...
    public boolean checkHuGuangDong(Avatar avatar, Integer cardIndex) {
        ...
        int flag = normalHuPai.checkGDhu(paiList, avatar.avatarVO.getPengArray(), avatar.avatarVO.getGangArray(),
